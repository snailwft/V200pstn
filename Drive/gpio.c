...
void gpio_init(void)
{
	...
	LPC_IOCON->PIO1_5 = 0XD0;							//把p1.5设置为数字IO脚输入				PSTN_DOUT
...
{      
	...
	{
		Output >>= 1; //先收低位
		if(GET_BIT(LPC_GPIO1, DATA, 5) > 0) 		//高电平
			Output |= 0x80;				
		wait(); 			//位间延时	
		wait();		
	}
...
{
	CLR_BIT(LPC_GPIO1,IE,5); //设置P1.5中断屏蔽
}
...
void PIOINT1_IRQHandler(void)
{
	...
	uint8 redata;
	//检测是否是该中断产生
	if (GET_BIT(LPC_GPIO1, MIS, 5)!=0)	      // 检测P1.5引脚产生的中断 PSTN_DOUT_MCU 下降沿触发中断
	{		
		//屏蔽中断
		CLR_BIT(LPC_GPIO1,IE,5); //设置P1.5中断屏蔽
		LPC_TMR16B0->TCR = 0x01;  //启动Timer16 0
