...
/*--------------------- Clock Configuration ----------------------------------
//
// <e> Clock Configuration	 时钟配置
//   <e1> System Clock Setup 系统控制和状态寄存器 (SCS)
//     <e2> System Oscillator Enable 主晶振使能
//       <o3.1> Select System Oscillator Frequency Range  OSCRANGE:主晶振频率范围选择
//                     <0=> 1 - 20 MHz
//                     <1=> 15 - 25 MHz
//     </e2>
//
...
//                     <0=> IRC Oscillator	  内部RC晶振
//                     <1=> System Oscillator 主晶振
//                     <2=> WDT Oscillator	  RTC晶振
//                     <3=> Invalid
//     <e7> Use System PLL
//                     <i> F_pll = M * F_in
...
//                     <1-32><#-1>
//       <o8.5..6>   P: PLL Divider Selection	PSEL: PLL分频选择
//                     <0=> 2
//                     <1=> 4
//                     <2=> 8
//                     <3=> 16
...
#define SYSOSCCTRL_Val        0x00000000   //第0位默认为晶振旁路: 第1位 0=0~20Mhz晶振输入, 1=15~50Mhz晶振输入

#define WDTOSC_SETUP          0
#define WDTOSCCTRL_Val        0x000000A0   // 看门狗时钟

#define SYSPLLCLKSEL_Val      0x00000000   // PLL输入时钟选择值
#define SYSPLL_SETUP          1
#define SYSPLLCTRL_Val        0x00000023   //设置M=4;P=2; FCLKOUT=12*4=48Mhz

#define MAINCLKSEL_Val        0x00000003   // 主时钟选择值
#define SYSAHBCLKDIV_Val      0x00000001   // 系统AHB时钟分频器
 ...
 * @return none
 *
 * @brief  Setup the microcontroller system. 微控制器配置，初始化系统并更新系统时钟频率
 *         Initialize the System.
 */
void system_init (void)
{
  ...
  LPC_SYSCON->PDRUNCFG     &= ~(1 << 5);          /* Power-up System Osc  系统振荡器上电      */
  LPC_SYSCON->SYSOSCCTRL    = SYSOSCCTRL_Val;	  /*第0位默认为晶振旁路: 第1位 0=0~20Mhz晶振输入, 1=15~50Mhz晶振输入 */
  for (i = 0; i < 200; i++) __NOP();			  //等待振荡器稳定
  LPC_SYSCON->SYSPLLCLKSEL  = SYSPLLCLKSEL_Val;   /* Select PLL Input  PLL时钟源选择“IRC 振荡器”       */
  LPC_SYSCON->SYSPLLCLKUEN  = 0x01;               /* Update Clock Source  更新PLL选择时钟源    */
  LPC_SYSCON->SYSPLLCLKUEN  = 0x00;               /* Toggle Update Register 先写0，再写1达到更新时钟源的目的（数据手册规定）  */
  LPC_SYSCON->SYSPLLCLKUEN  = 0x01;
  while (!(LPC_SYSCON->SYSPLLCLKUEN & 0x01));     /* Wait Until Updated   确定时钟源更新后向下执行    */
#if (SYSPLL_SETUP)                                /* System PLL Setup         */
  LPC_SYSCON->SYSPLLCTRL    = SYSPLLCTRL_Val;	  //设置M=4;P=2; FCLKOUT=12*4=48Mhz
  LPC_SYSCON->PDRUNCFG     &= ~(1 << 7);          /* Power-up SYSPLL    PLL上电      */
  while (!(LPC_SYSCON->SYSPLLSTAT & 0x01));	      /* Wait Until PLL Locked 确定PLL锁定以后向下执行   */
#endif
#endif
#if (WDTOSC_SETUP)                                /* Watchdog Oscillator Setup*/
...
#endif
  LPC_SYSCON->MAINCLKSEL    = MAINCLKSEL_Val;     /* Select PLL Clock Output 主时钟源选择PLL后的时钟 */
  LPC_SYSCON->MAINCLKUEN    = 0x01;               /* Update MCLK Clock Source 更新主时钟源*/
  LPC_SYSCON->MAINCLKUEN    = 0x00;               /* Toggle Update Register  先写0，再写1达到更新时钟源的目的（数据手册规定） */
  LPC_SYSCON->MAINCLKUEN    = 0x01;
  while (!(LPC_SYSCON->MAINCLKUEN & 0x01));       /* Wait Until Updated  确定主时钟锁定以后向下执行     */
#endif

  LPC_SYSCON->SYSAHBCLKDIV  = SYSAHBCLKDIV_Val;	  //AHB时钟分频值为1，使AHB时钟设置为48Mhz
  LPC_SYSCON->SYSAHBCLKCTRL = AHBCLKCTRL_Val;	  /* 系统时钟IOCON使能，此位不使能大部分IO不可用 */	 //使能GPIO时钟
